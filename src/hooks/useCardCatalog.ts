...
const CARD_CATALOG_STORAGE_KEYS: Record<CardCatalogMode, string> = {
  'with-effects': 'card_catalog_v5_with_effects',
  'no-effects': 'card_catalog_v5_no_effects'
};
...
async function fetchCardCatalog(includeEffects: boolean) {
  ...
  const res = await fetch(`${BACKEND_URL}/api/cards/meta?includeEffects=${includeEffects ? '1' : '0'}&catalogVersion=5`, {
    cache: 'no-store'
  });
